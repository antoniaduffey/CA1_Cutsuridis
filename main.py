...
import importlib
import netfcns
from model import Model_cell, AACell, BasketCell, BistratifiedCell, BurstCell, OLMCell, PyramidalCell, StimCell
import pickle
...
# scc has an odd way of accounting for command line arguments
if (result.stdout.decode('utf-8')[:3] == "scc"):
    argadd = 2
    startlen = 5
        ...
        if len(sys.argv) > (2*argadd+startlen):
            ...
            if len(sys.argv) > (3*argadd+startlen):
                ...
                if len(sys.argv) > (4*argadd+startlen):
                    connect_random_low_start_ = float(
                        sys.argv[4*argadd+startlen])
...
poplist = []
poplist.append(CellPop(num=100*network_scale, order=0,
                       classtype="PyramidalCell", popname="PyramidalCell", isart=0))
poplist.append(CellPop(num=2, order=1, popname="BasketCell",
                       classtype="BasketCell", isart=0))
poplist.append(CellPop(num=1, order=2, popname="AACell",
                       classtype="AACell", isart=0))
poplist.append(CellPop(num=1, order=3, popname="BistratifiedCell",
                       classtype="BistratifiedCell", isart=0))
poplist.append(CellPop(num=1, order=4, popname="OLMCell",
                       classtype="OLMCell", isart=0))
poplist.append(CellPop(num=100*network_scale, order=5,
                       popname="CA3Cell", classtype="StimCell", isart=1)) 
poplist.append(CellPop(num=20*network_scale, order=6,
                       popname="ECCell", classtype="StimCell", isart=1))
poplist.append(CellPop(num=10*network_scale, order=7,
                       popname="SEPCell", classtype="BurstCell", isart=1))
...
h.nrnmpi_init()
# Set up a parallel net manager for all the cells
...
#    for j in range(int(pop.num)):    # in serial, make all cells on one core
    # in parallel, make every nth cell on one core
    for j in range(int(pop.gidst), int(pop.gidend)+1):
        if (pc.gid_exists(j)):
            if (pc.id() == 0 and printflag > 1):
                print("newcell =" + pop.classtype + "." + pop.classtype + "(int("+str(j)+"))")
            exec("newcell =" + pop.classtype + "." + pop.classtype + "(int("+str(j)+"))")
            if (pop.isart == 1):
            ...
            if (j == 206):
                # ranlist.o(i) corresponds to
                ranlist.append(h.RandomStream(j+500))
            else:
                # ranlist.o(i) corresponds to
                ranlist.append(h.RandomStream(j))
            i += 1
            pcst += pc.nhost()
            core_i += 1
            coretype_i += 1
            ...
            # associate cell's gid with its spike detector
            pc.cell(newcell.gid, nc)

    pop.core_en = core_i-1


# Calculate totals of cells
# total number of real cells
ncell = sum([o.num for o in poplist if o.isart == 0])
nstim = sum([o.num for o in poplist if o.isart == 1]
...
if network_scale == 1:
    ...
    # "Weights/pattsN100S20P5.dat"    # already stored patterns
    FPATT = "Weights/patts"+netfile+".dat"
    # "Weights/pattsN100S20P5.dat"    # new patterns to store
    FSTORE = "Weights/patts"+netfile+".dat"
else:
    netfile = 'N100S20P5'
    FCONN = "Weights/wgtsN100S20P5scaled.dat"  # "Weights/wgtsN100S20P5.dat"
    # "Weights/pattsN100S20P5.dat"    # already stored patterns
    FPATT = "Weights/pattsN100S20P5scaled.dat"
    # "Weights/pattsN100S20P5.dat"    # new patterns to store
    FSTORE = "Weights/pattsN100S20P5scaled.dat"
# %%
...
# Postsynaptic type      Presynaptic type  number of connections (at least 1)
connlist = []
...
connlist.append(popConn(popname="BasketCell",    prepop="CA3Cell", prenum=max(
    [pop_by_name["CA3Cell"].num * scaleEScon, 1]), type="AMPA", weight=EIWGT, delay=EIDEL, synst=EI_CA3, synend=EI_CA3+3))
connlist.append(popConn(popname="AACell",        prepop="CA3Cell", prenum=max(
    [pop_by_name["CA3Cell"].num * scaleEScon, 1]), type="AMPA", weight=EIWGT, delay=EIDEL, synst=EI_CA3, synend=EI_CA3+3))  # CA3_AAC
connlist.append(popConn(popname="BistratifiedCell", prepop="CA3Cell", prenum=max(
    [pop_by_name["CA3Cell"].num * scaleEScon, 1]), type="AMPA", weight=EIWGT, delay=EIDEL, synst=EI_CA3, synend=EI_CA3+3))  # CA3_BSC
...
connlist.append(popConn(popname="BasketCell",    prepop="ECCell", prenum=max(
    [pop_by_name["ECCell"].num * scaleEScon, 1]), type="AMPA", weight=EIWGT, delay=EIDEL, synst=EI_EC, synend=EI_EC+1))  # EC_BC
connlist.append(popConn(popname="AACell",        prepop="ECCell", prenum=max(
    [pop_by_name["ECCell"].num * scaleEScon, 1]), type="AMPA", weight=EIWGT, delay=EIDEL, synst=EI_EC, synend=EI_EC+1))  # EC_AAC

connlist.append(popConn(popname="BasketCell",    prepop="SEPCell",
                        prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGT, delay=SEPDEL, synst=II_SEP, synend=II_SEP+1))  # SEP_BC
connlist.append(popConn(popname="AACell",        prepop="SEPCell",
                        prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGT, delay=SEPDEL, synst=II_SEP, synend=II_SEP+1))  # SEP_AAC

if keepoldtypo:
    connlist.append(popConn(popname="BasketCell", prepop="SEPCell",
                            prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGTL, delay=SEPDEL, synst=II_SEP, synend=II_SEP+1))  # SEP_BSC
    connlist.append(popConn(popname="AACell",       prepop="SEPCell",
                            prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGTL, delay=SEPDEL, synst=IO_IN, synend=IO_IN))  # SEP_OLM
else:
    connlist.append(popConn(popname="BistratifiedCell", prepop="SEPCell",
                            prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGTL, delay=SEPDEL, synst=II_SEP, synend=II_SEP+1))  # SEP_BSC
    connlist.append(popConn(popname="OLMCell",       prepop="SEPCell",
                            prenum=pop_by_name["SEPCell"].num, type="GABAA", weight=SEPWGTL, delay=SEPDEL, synst=IO_IN, synend=IO_IN))  # SEP_OLM

connlist.append(popConn(popname="PyramidalCell", prepop="PyramidalCell", prenum=max(
    [1 * scaleEScon, 1]), type="AMPA", weight=Pcell2Pcell_weight, delay=Pcell2Pcell_delay, synst=E_PC, synend=E_PC))  # PC_PC
connlist.append(popConn(popname="BasketCell",    prepop="PyramidalCell", prenum=max(
    [pop_by_name['PyramidalCell'].num * scaleEScon, 1]), type="AMPA", weight=Pcell2Bcell_weight, delay=Pcell2Bcell_delay, synst=EI_PC, synend=EI_PC+1))  # PC_BC
connlist.append(popConn(popname="BistratifiedCell", prepop="PyramidalCell", prenum=max(
    [pop_by_name['PyramidalCell'].num * scaleEScon, 1]), type="AMPA", weight=Pcell2AAcell_weight, delay=Pcell2AAcell_delay, synst=EI_PC, synend=EI_PC+1))  # PC_BSC
connlist.append(popConn(popname="AACell",        prepop="PyramidalCell", prenum=max(
    [pop_by_name['PyramidalCell'].num * scaleEScon, 1]), type="AMPA", weight=Pcell2BScell_weight, delay=Pcell2BScell_delay, synst=EI_PC, synend=EI_PC+1))  # PC_AAC
connlist.append(popConn(popname="OLMCell",       prepop="PyramidalCell", prenum=max(
    [pop_by_name['PyramidalCell'].num * scaleEScon, 1]), type="AMPA", weight=Pcell2OLMcell_weight, delay=Pcell2OLMcell_delay, synst=EO_PC, synend=EO_PC+1))  # PC_OLM

connlist.append(popConn(popname="PyramidalCell", prepop="BasketCell", prenum=2, type="GABAA",
                        weight=Bcell2Pcell_weight, delay=Bcell2Pcell_delay, synst=I_BC, synend=I_BC))  # BC_PC
connlist.append(popConn(popname="BasketCell",    prepop="BasketCell", prenum=1, type="GABAA",
                        weight=Bcell2Bcell_weight, delay=Bcell2Bcell_delay, synst=II_SAME, synend=II_SAME))  # BC_BC
connlist.append(popConn(popname="BistratifiedCell", prepop="BasketCell", prenum=2, type="GABAA",
                        weight=Bcell2BScell_weight, delay=Bcell2BScell_delay, synst=II_OPP, synend=II_OPP))  # BC_BSC
...
connlist.append(popConn(popname="PyramidalCell", prepop="AACell", prenum=1, type="GABAA",
                        weight=AAcell2Pcell_weight, delay=AAcell2Pcell_delay, synst=I_AAC, synend=I_AAC))  # AAC_PC
connlist.append(popConn(popname="PyramidalCell", prepop="BistratifiedCell", prenum=1, type="GABAA",
                        weight=BScell2Pcell_weight, delay=BScell2Pcell_delay, synst=I_BSC, synend=I_BSC+5))  # BSC_PC
connlist.append(popConn(popname="PyramidalCell", prepop="BistratifiedCell", prenum=1, type="GABAB",
                        weight=BScell2Pcell_GABAB_weight, delay=BScell2Pcell_delay, synst=I_BSC+6, synend=I_BSC+11))  # BSC_PC
...
connlist.append(popConn(popname="BasketCell",    prepop="BistratifiedCell", prenum=1, type="GABAA",
                        weight=BScell2Bcell_weight, delay=BScell2Bcell_delay, synst=II_OPP, synend=II_OPP))  # BSC_BC

connlist.append(popConn(popname="PyramidalCell", prepop="OLMCell", prenum=1, type="GABAA",
                        weight=OLMcell2Pcell_weight, delay=OLMcell2Pcell_delay, synst=I_OLM, synend=I_OLM+1))  # OLM_PC
connlist.append(popConn(popname="PyramidalCell", prepop="OLMCell", prenum=1, type="GABAB",
                        weight=OLMcell2Pcell_GABAB_weight, delay=OLMcell2Pcell_delay, synst=I_OLM+2, synend=I_OLM+3))  # OLM_PC
...
for conn in connlist:
    conn.connsMade = netfcns.connectcells(cells, ranlist, nclist, pop_by_name, conn.popname, conn.prepop,
                                          synstart=conn.synst, synend=conn.synend, npresyn=conn.prenum, weight=conn.weight, delay=conn.delay, pc=pc)
    # if (printflag>1):
...
# EC input to PCs
ncelist = netfcns.connectEC(FPATT, ECPATT, NPATT, E_EC,
                            2, cells,  pop_by_name, pc)  # restore existing pattern
# CA3 input to PCs
ncslist = netfcns.connectCA3(FCONN, C_P, EM_CA3, EN_CA3, cells, pop_by_name,
                             connect_random_low_start_, pc)  # with modifiable synapses
# %%
...
netfcns.mkcue(FPATT, CPATT, CFRAC, NPATT, SPATT, cells, ranlist,
              pop_by_name, pc)  # cue from already stored pattern
...
# number of cells
num2pick = int(percentDeath*pop_by_name["PyramidalCell"].num)

deadlist = []

new_random = h.Random(400)
new_random.discunif(pop_by_name["PyramidalCell"].gidst,
                    pop_by_name["PyramidalCell"].gidend)
...
# if (batchflag==1):
if (pc.id() == 0 and printflag > 0):
    print("Now running simulation at scale = ", network_scale,
          " for time = ", SIMDUR, " with scaleEScon = ", scaleEScon)
...
data2save = {'dt': h.dt, 'tstop': h.tstop, 'netfile': netfile, 'simname': simname, 'performance': perf,
             'electrostim': electrostim, 'percentDeath': percentDeath, 'network_scale': network_scale}
...
pc.barrier()  # wait for all hosts to get to this point
if (pc.id() == 0 and plotflag > 0):
    pf.plotresults(params)  # pf.plotresults("guitar")
